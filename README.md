...
기능 구현 및 디버깅까지 전부 포함하여 약 한 달의 기간이 소요되었다.<br>
아직 개발이 능숙하지 않은데 욕심이 너무 앞서면 제작 기간이 오래 걸릴 것 같다는 점과 더불어<br>
'커피'에 본격 입문하기 전에 간단하게 좀 가벼우면서도 핵심적인 정보가 담겨져 있으면 좋겠다 라는 생각을 했다.<br>
전체적인 설계와 기능 후현에 반 정도 걸리고, 반은 거의 디버깅에 시간이 소요되었다.
<br>
<br>
### * 자료 참고
...
디자이너로서 자료를 자체 제작하고자 하니 목표 기간 보다 꽤나 걸릴 것 같았고,<br>
...
<br>
-사용자가 선호하는 맛과 느낌에 따라서 개인 맞춤형 원두의 정보를 제공해줍니다.
<br>
...
그동안 공부해왔던 udemy 인강 + 생활코딩 인강의 지식들을 개인적으로 공부정리한 문서들과 구글링, ChatGpt 3.5를 전반적으로 활용하였다.<br>
...
또한 해당 케이스가 챗 지피티에 물어보면 해결되지 않는 방법을 제시해 주고 구글링으로는 해법을 찾기가 어려워서<br>
해당 오류 디버깅에 생각보다 많은 시간을 소요하게 되었는데<br>
너무 외적인 방법에 의존을 하면 안 되고 에러문을 정확히 인지하고 분석하고 추론하는 것이<br>
...
메뉴 바의 해당 드롭박스의 링크가 해당 링크항목만 기입되면 해결이 되지만, 이 경우 말 그대로 클릭한 해당 항목의 js컴포넌트만 출력해서 보여준다.<br>
나는 Type.js를 전체를 보여주면서 에스프레소, 라떼, 브루드 커피를 클릭하면, 해당 위치로 스크롤이 되는 기능을 원했다.<br>
<br>
답은 지피티와의 끈질긴 지속적인 상호작용을 통해 얻어내었는데<br>
```javascript
  useEffect(() => {
    if (location.hash) {
...
그래서 일종의 선생님 역할이라 생각하고 ChatGpt3.5에게 많이 물어보았다.<br>
